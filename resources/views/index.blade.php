        ...
        const data = [
            {
                ...
                percentage: {{ $student->total_percentage_score }},
                subjects: [
                        @php $scores = $student->scores ?: []; @endphp
                        @foreach($scores as $s)
                        @php
                            $balls = json_decode($s->scores, true);
                            $balls = $balls ?: [];
                        @endphp
                    {
                        subject: "{{ $s->subject->name ?? "" }}",
                        percentage: {{ $s->score_percentage ?? 0 }},
                        color: "bg-cyan-600",
                        lessons: [
                                @for($i = 0; $i < count($balls); $i ++)
                                @if(count($balls)-1 == $i )
                            { lesson: "Yakuniy", mark: "@if($balls[$i] >= 0) {{ $balls[$i] }} @else {{ "➖" }} @endif" },
                                @else
                            { lesson: "{{ $i+1 }}-topshiriq", mark: "@if($balls[$i] >= 0) {{ $balls[$i] }} @else {{ "➖" }} @endif" },
                            @endif
                            @endfor
                        ],
                    },
            ...
            },
            {
                ...
                percentage: {{ $student->total_percentage_attendance ?? 0 }},
                subjects: [
                        @foreach($scores as $s)
                        @php
                            $attends = json_decode($s->attendances, true);
                            $attends = $attends ?: [];
                        @endphp
                    {
                        subject: "{{ $s->subject->name ?? "" }}",
                                            ...
                                            <td colspan="3" class="p-4">
                                                <div class="overflow-x-auto">
                                                    <table class="w-full text-sm text-left text-gray-500">
                                                        <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                                                            <tr>
                                                                <th scope="col" class="px-4 py-2">Sana</th>
